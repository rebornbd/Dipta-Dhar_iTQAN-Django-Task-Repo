...
export default function UpdatePhone() {
  ...
  useEffect(() => {
    // get specific phone
    Axios(makeConfig('get', `${CONSTANT.APP_BASE_URL}/api/phones/${phoneId}/`))
    .then(res => res.data)
    .then(res => {
      setPhones(res);
  ...
  const handleForm = () => {
    ...
    Axios(makeConfig('put', `${CONSTANT.APP_BASE_URL}/api/phones/phone/${phoneId}/`, data))
    .then(res => res.data)
    .then(res => {
      history.push('/phone')
    })
  ...
  return (brands.length<=0 && phones.length<=0)
    ? <>
      <Header />
      <Loading />
      <Footer />
          ...
          <div className={styles.item}>
            ...
            <select className={styles.brandName} onChange={(e) => setBrandId(e?.target?.value)}>
              ...
              {brands && brands.map((brand, index) => {
                ...
                ? <option value={brand?.id} key={index} selected>{brand?.name}</option>
          ...
          <div className={styles.item}>
            ...
            <button 
              ...
              className={formValidity() 
                ...
                : `${styles.createBtn}`}>Update Mobile Entry</button>
          </div>
        </div>
      </div>
      <Footer />
